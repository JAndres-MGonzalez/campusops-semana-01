...
export async function handleCampusOps(request, response, url, { send, readJson, scenario }) {
  if (request.method === 'POST' && url.pathname === '/v1/session/login') {
    ...
    return send(response, 200, { actorId: input.actorId, role: actors[input.actorId], accessToken: validToken, refreshToken: refreshToken0, expiresIn: 60 });
  }
